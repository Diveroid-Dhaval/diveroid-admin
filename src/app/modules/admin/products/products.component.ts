...
export class ProductsComponent implements OnInit {
    ...
    private getProductList(): void {
        ...
        this.baseService.get(Apiurl.productList, params).subscribe(
            (response: any) => {
...
-----------------------------------*/
    ...
    defineForm() {
        this.productForm = this.fb.group({
            ...
            PeriodValue: ['', [Validators.required, Validators.min(0)]],
        });
    }
    ...
    setFormValue(data: any) {
        this.id = data.id;
        this.productForm.controls.PackageName.setValue(data.PackageName);
        this.productForm.controls.PaymentType.setValue(data.PaymentType);
        this.productForm.controls.PaymentUnit.setValue(data.PaymentUnit);
        this.productForm.controls.PaymentAmount.setValue(data.PaymentAmount);
        this.productForm.controls.PeriodType.setValue(data.PeriodType);
        this.productForm.controls.PeriodValue.setValue(data.PeriodValue);
    }
    /**
    ...
    saveForm() {
        ...
        if (this.isEdit) {
            this.baseService
                .put(Apiurl.productList + this.id, this.productForm.value)
                .subscribe(
                    (res: any) => {
                        if (res) {
                            this.toastService.showToastMessage(
            ...
            this.baseService
                .post(Apiurl.productList, this.productForm.value)
                .subscribe(
                    (res: any) => {
                        if (res) {
                            this.toastService.showToastMessage(
    ...
    deleteRecord(id: number) {
        ...
        confirmation.afterClosed().subscribe((dialogResult) => {
            if (dialogResult === true) {
                ...
                this.baseService.delete(Apiurl.productList + id).subscribe(
                    (response: any) => {
                        if (response) {
                            // this.spinnerService.show();
                            this.toastService.showToastMessage(
