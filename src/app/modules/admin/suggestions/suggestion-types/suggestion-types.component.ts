...
@Component({
    selector: 'app-suggestion-types',
    templateUrl: './suggestion-types.component.html',
    styleUrls: ['./suggestion-types.component.scss'],
})
export class SuggestionTypesComponent implements OnInit {
    ...
    constructor(
        ...
        this.searchUpdater
            ...
            .subscribe(() => this.getSuggestionTypeList());
    }
    ...
    private getSuggestionTypeList(): void {
        ...
        this.baseService.get(Apiurl.suggestionTypeList, params).subscribe(
            (response: any) => {
                this.loader.hideLoader();
                if (response) {
                    this.dataSource.data = response.data.suggestionType;
    ...
    private initColumns(): void {
        this.columns = ['id', 'Type', 'Thumb', 'Language', 'action'];
    }
...
-----------------------------------*/
    ...
    setFormValue(data: any) {
        this.id = data.id;
        this.suggestionForm.controls.Type.setValue(data.Type);
        this.suggestionForm.controls.LanguageId.setValue(data.LanguageId);
    ...
    saveForm() {
        ...
        const formValues = {
            Type: this.suggestionForm.value.Type,
            LanguageId: this.suggestionForm.value.LanguageId,
        ...
        const APIURL = this.isEdit
            ? Apiurl.suggestionTypeList + this.id
            : Apiurl.suggestionTypeList;
    ...
    deleteRecord(id: number) {
        const confirmation = this.dialog.open(CommonDeleteModalComponent, {
            data: {
                ...
                message:
                    'Are you sure you want to delete this suggestion type?',
            },
            width: '30%',
        });
        confirmation.afterClosed().subscribe((dialogResult) => {
            if (dialogResult === true) {
                ...
                this.baseService
                    .delete(Apiurl.suggestionTypeList + id)
                    .subscribe(
                        (response: any) => {
                            if (response) {
                                // this.spinnerService.show();
